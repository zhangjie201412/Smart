...
//#define CUSTOMIZE_VOLUME_ADJUST	1
...
#ifdef CUSTOMIZE_VOLUME_ADJUST
static int snd_soc_nau8810_put_volsw(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	...
	if (val == 0x39) val = 0x39;

	printk("set_volsw=0x%x\n", val);
	reg = snd_soc_read(codec, NAU8810_REG_SPKGAIN) & (~0x3f);
	snd_soc_write(codec, NAU8810_REG_SPKGAIN, reg | val);

	return 0;
}

static int snd_soc_nau8810_get_volsw(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	u16 val;

	val = snd_soc_read(codec, NAU8810_REG_SPKGAIN) & (0x3f);
	printk("get_volsw=0x%x\n", val);
	val = (val >= 4) ? 4  : (3 -  val);
...
#ifdef CUSTOMIZE_VOLUME_ADJUST
SOC_SINGLE_EXT_TLV("Speaker Volume", NAU8810_REG_SPKGAIN, NAU8810_SPKGAIN_SFT, 0x39, 0,
                           snd_soc_nau8810_get_volsw,
                           snd_soc_nau8810_put_volsw, spk_tlv),
#else
