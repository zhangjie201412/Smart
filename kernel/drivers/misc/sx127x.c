...
#define SX127X_FSKOOKREG(addr) (addr | SX127X_FSKOOKREG_MASK)
...
#define SX127X_RESET_PIN                                                NUC970_PG5
#define SX127X_DIO0_PIN                                                 NUC970_PG4
...
#else
	size_t maxtransfer = 128;//spi_max_transfer_size(spi);
#endif
	int ret;
	unsigned readlen;
...
static int sx127x_fifo_writepkt(struct spi_device *spi, void *buffer, u8 len)
{
	...
	dev_info(&spi->dev, "fifo write: %d, addr: %d\n", len, addr);
...
static ssize_t sx127x_pkgcfg1_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sx127x *data = dev_get_drvdata(dev);
	u8 config1;
	int ret;
	...
	mutex_lock(&data->mutex);
	sx127x_reg_read(data->spidevice, SX127X_REG_FSKOOK_PACKETCONFIG1, &config1);
	ret = sprintf(buf, "0x%02x\n", config1);
	mutex_unlock(&data->mutex);
	return ret;
}

static ssize_t sx127x_pkgcfg1_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct sx127x *data = dev_get_drvdata(dev);
	int val;
	...
	sscanf(buf, "%02x", &val);
	mutex_lock(&data->mutex);
	sx127x_reg_write(data->spidevice, SX127X_REG_FSKOOK_PACKETCONFIG1, val);
	mutex_unlock(&data->mutex);
	return count;
}

static DEVICE_ATTR(pkgcfg1, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH, sx127x_pkgcfg1_show, sx127x_pkgcfg1_store);

static ssize_t sx127x_padac_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sx127x *data = dev_get_drvdata(dev);
	u8 adrs;
	int ret;
	...
	mutex_lock(&data->mutex);
	sx127x_reg_read(data->spidevice, SX127X_REG_PADAC, &adrs);
	ret = sprintf(buf, "0x%02x\n", adrs);
	mutex_unlock(&data->mutex);
	return ret;
}

static ssize_t sx127x_padac_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct sx127x *data = dev_get_drvdata(dev);
	int val;
	...
	sscanf(buf, "%02x", &val);
	mutex_lock(&data->mutex);
	sx127x_reg_write(data->spidevice, SX127X_REG_PADAC, val);
	mutex_unlock(&data->mutex);
	return count;
}

static DEVICE_ATTR(padac, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH, sx127x_padac_show, sx127x_padac_store);
...
static unsigned int sx127x_dev_poll(struct file *filp, poll_table *wait)
{
	struct sx127x *data = filp->private_data;
	unsigned int mask = 0;

	mutex_lock(&data->mutex);
	poll_wait(filp, &data->readwq, wait);
	poll_wait(filp, &data->writewq, wait);

	if(kfifo_len(&data->out) > 0) {
		mask |= POLLIN | POLLRDNORM;
	}
	if(!kfifo_is_full(&data->out)) {
...
static int sx127x_probe(struct spi_device *spi)
{
	...
	ret = gpio_request(SX127X_RESET_PIN, "SX127X_RESET_PIN");
	if(ret) {
		pr_err("%s: failed to request rf reset pin\n", __func__);
		ret = -ENOMEM;
		goto err_resetgpio;
	}
	gpio_direction_output(SX127X_RESET_PIN, 1);
	msleep(100);
	gpio_set_value(SX127X_RESET_PIN, 0);
	msleep(2);
	gpio_set_value(SX127X_RESET_PIN, 1);
	msleep(100);
	...
	dev_info(&spi->dev, "chip version %x\n",(unsigned) version);

	ret = gpio_request(SX127X_DIO0_PIN, "SX127X_DIO0_PIN");
	if(ret) {
		pr_err("%s: failed to request dio0 pin\n", __func__);
		ret = -EINVAL;
		goto err_irq;
	}
	gpio_direction_input(SX127X_DIO0_PIN);
	data->irq = gpio_to_irq(SX127X_DIO0_PIN);
	if(data->irq < 0) {
		...
		pr_info("%s: irq = %d\n", __func__, data->irq);
		ret = request_irq(data->irq, sx127x_irq, IRQF_TRIGGER_RISING, "sx127x_irq", data);
		if(ret < 0) {
			pr_err("%s: failed to request sx127x irq\n", __func__);
		}
	}
	...
	ret = device_create_file(data->chardevice, &dev_attr_pkgcfg1);
	ret = device_create_file(data->chardevice, &dev_attr_padac);
...
static int sx127x_remove(struct spi_device *spi)
{
	...
	device_remove_file(data->chardevice, &dev_attr_pkgcfg1);
	device_remove_file(data->chardevice, &dev_attr_padac);
