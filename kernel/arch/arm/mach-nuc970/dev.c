...
static int nuc970_keymap[] = {
	KEY(0, 0, KEY_KPPLUSMINUS),	KEY(0, 1, KEY_VOLUMEUP),
	KEY(1, 0, KEY_KPEQUAL),		KEY(1, 1, KEY_VOLUMEDOWN),
	KEY(2, 0, KEY_MINUS),			KEY(2, 1, KEY_EQUAL),
};
...
#ifdef CONFIG_BLADE_WATCHDOG
static struct platform_device nuc97x_blade_wdt = {
	.name = "blade-watchdog",
};
#endif

#ifdef CONFIG_GPRS_POWER_INIT
static struct platform_device gprs_power_device = {
	.name = "gprs_power",
};
#endif
...
void __init nuc970_platform_init(struct platform_device **device, int size)
{
    ...
    nuc970_mfp_set_port_g(5, 0);
    nuc970_mfp_set_port_g(4, 0);
}
